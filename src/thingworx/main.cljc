(ns thingworx.main
  "Kotodama WASM entrypoint for the Thingworx clean-room actor (L4) — Clojure port.
  ...
  facts (`thingworx.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "thingworx")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"thingworx.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "thingworx-compat" :tier tier :entities entities} 200])
...
(defn start! [] :thingworx-compat/ready)
